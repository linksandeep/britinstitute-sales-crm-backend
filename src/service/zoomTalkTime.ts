...
export const sumZoomTalkTime = (calls: TalkTimeCall[], timeZone: string, now = new Date()) => {
  ...
  const daily = { date: range.today, talk_time_seconds: 0, connected_calls: 0 };
  const weekly = { from: range.from, to: range.today, talk_time_seconds: 0, connected_calls: 0 };
